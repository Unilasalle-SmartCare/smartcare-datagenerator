    ...
    methods: {
        ...
        setStress() {
            try {
                ...
                this.geraErro(`Houve um erro interno (0x85) - ${error}`)
            }
        },
        geraErro(erro) {
            this.erro.texto = erro;
            this.erro.show = true;
        },
        checkStress(data) {
            try {
                var dataS = _.filter(dados, function(o) {
                    if (o.date == data) return o;
                });
                if (dataS.length) {
                    if (dataS[0].stress) {
                        return 0
                ...
                this.geraErro(`Houve um erro interno (0x27) - ${error}`)
            }
        },
        datasOrdenadas() {
            try {
                ...
                this.geraErro(`Erro ao ordenadar as datas (0xb1) - ${error}`)
            }
        },
        save() {
            try {
        ...
        deletarDados() {
            try {
                ...
                this.geraErro(`Houve um erro interno (0xd7) - ${error}`)
            }
        },
        uploadJson() {
